...
const DeletePage = () => {
  ...
  if (!tag)
    return (
      <div>
        ...
        <h2 style={{ marginTop: "20px" }}>Playlist {tag_id} not found</h2>
      </div>
    );
  ...
  if (delete_success)
    return (
      <div>
        <h2>Successfully deleted playlist "{tag.name}"</h2>
  ...
  return (
    <div>
      <Button href={`/playlists/${tag_id}`} variant="contained">
        Back
      </Button>
      <h2 style={{ marginTop: "10px" }}>Are you sure you want to delete the "{tag.name}" playlist?</h2>
