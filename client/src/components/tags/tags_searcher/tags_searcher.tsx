...
const TagSearcher: React.FC<IProps> = (props: IProps) => {
  const { getRootProps, getInputLabelProps, getInputProps, getTagProps, getListboxProps, getOptionProps, groupedOptions, value, focused, setAnchorEl } =
    useAutocomplete({
    ...
    <Root>
      ...
      {groupedOptions.length > 0 ? (
        <Listbox
          {...getListboxProps()}
          onClick={(event: any) => {
            const tag_name = event.target.innerText.split("\n")[0];
            console.log("pressed on", tag_name);
            props.add_tag(tag_name)
          }}
        >
