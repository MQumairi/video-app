...
test("can load directories", async () => {
  const root_res = await request.get("/api/directories/test%2Fdata");
  expect(root_res.statusCode).toBe(200);
  expect(root_res.body).toMatchObject(root_dir_object);

  const dir_1_res = await request.get("/api/directories/test%2Fdata%2Fdir_1");
  expect(dir_1_res.statusCode).toBe(200);
  expect(dir_1_res.body).toMatchObject(dir_1_object);

  const sub_dir_res = await request.get("/api/directories/test%2Fdata%2Fdir_1%2Fsub_dir");
